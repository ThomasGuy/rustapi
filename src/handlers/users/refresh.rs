...
pub async fn refresh_handler(
    ...
    // 8. Issue new tokens
    let new_access = encode_token(user.id, keys, 15, TokenType::Access, &state)?; // 15mins
    let new_refresh_raw = encode_token(user.id, keys, 10080, TokenType::Refresh, &state)?; // 7 days
